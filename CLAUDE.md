...
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- |
| `ci.yml`                | Python pipeline regression suite (40 tests, `tests/test_pipeline.py`) + `build_docs.py --check`                                 | Pipeline correctness on demo-kit fixture; /docs page must be in sync with install scripts.            |
| `publish-landing.yml`   | Auto-runs `installer/publish-landing.sh` on `main` pushes that touch `installer/**` or `demo-kit/data/**`. Smoke-checks live URLs. | Keeps the live site in sync with main. Without it, install.ps1 / /docs / /demo-data drift between merges. Needs `HERENOW_API_KEY` repo secret. |
...
| `landing-guard.yml`     | Validate-only mirror of the publish-landing.sh substitution pipeline; refuses to pass if `{{` placeholders survive in `index.html` / `dashboard-demo/index.html` | Prevents the literal-`{{BUILD_STAMP}}`-on-live-site regression class. Renamed from `pages.yml` on 2026-05-19 when the dead Pages-deploy steps were stripped (Pages was never enabled on the repo). |
...
- `landing-guard.yml` placeholder guard — don't disable it. `{{BUILD_STAMP}}` in `installer/index.html` is intentional; substitution happens at publish time via `installer/publish-landing.sh` or the workflow.
...
| See what's left to build / what's done | `dev/backlog.md`. Items are organized as Epics (E1.x, E2.x …) and Build increments (B9.16, B9.17, …). Build numbers are roughly chronological within an epic. The current era is B9.21+ (late May 2026, end-of-prototype). |
...
| Update what's at `passporttowealth.app/docs` | Source: `installer/docs/index.html`. Generated by `python3 installer/build_docs.py` from `installer/install.sh` + `install.ps1` + `requirements.txt`. The lists (packages, URLs, env vars, paths) are extracted; the descriptions are curated maps in `build_docs.py`. **Adding a new package, URL, or env var that's not in the curated maps fails CI.** Republish to live: `bash installer/publish-landing.sh` (it regenerates first). |
| Edit a legal page                      | **Don't, unless you're the user or counsel.** `installer/legal/{privacy,terms,dpa}.html` are engineering-drafted and marked "draft pending legal review." If a fix is genuinely required (e.g., a factual correction about what the code does), make the smallest possible change and keep the draft banner. New surfaces or reframing requires the user's go-ahead. |
| Update the dashboard template          | `skill/templates/site/index.html` (regenerate demo per `installer/README.md` → "The demo dashboard"). After any dashboard / chart / JS change, walk through `dev/SMOKE_CHECKS.md` (~30-second manual UI checklist — guards UX regressions tests don't cover). |
| Add or change a user-facing string     | `skill/prompts/user_facing_strings.md` is the canonical dictionary for every string the skill emits outside the install dialog. Add new strings there first; OP-8 lint enforces it. Add `<!--OP8-OK-->` on a line if it legitimately contains a banned word in a comment / link / heading. |
| Run the pipeline manually              | `FCB_WORKSPACE=/tmp/ws bash skill/scripts/refresh.sh --auto-confirm` after seeding `/tmp/ws/inbox/` |
| Touch the install scripts              | `installer/install.sh` (Mac) + `installer/install.ps1` (Windows) — keep in sync. After editing, regenerate `/docs` with `python3 installer/build_docs.py` (CI runs `--check` and fails the PR if it's stale). |
| Touch the Worker                       | `cloudflare-worker/src/index.js` — `wrangler deploy` from that directory. KV namespace `FCB_METRICS` (id `26eca96d62724b968eeefb17f7cd51e4`). Two endpoints: `/install` (anonymous telemetry) and `/` (feedback events that become GitHub issues). |
| Read advisor-facing docs               | `docs/advisor-onboarding.md` (pre-flight checklist before sending a client the install link), `docs/troubleshooting.md` (per-error-code playbook).                                  |
