...
| `docs/index.html` | Auto-generated docs page served at `https://passporttowealth.app/docs/`. Built by `build_docs.py`. See ["The /docs page"](#the-docs-page) below. |
| `build_docs.py` | Generator for `docs/index.html`. Parses `install.sh` + `install.ps1` for packages, URLs, env vars, and paths; cross-references curated descriptions; renders the docs page. CI runs `--check` mode and fails the build if the committed `docs/index.html` is out of sync. |
| `legal/` | Draft Privacy Policy, Terms of Service, and Data Processing Agreement (`privacy.html`, `terms.html`, `dpa.html`). All marked **draft pending legal review** — engineering drafts based on actual data flows. Don't edit without the user's go-ahead. See ["Legal pages"](#legal-pages) below. |
| `llms.txt` | Agent-discoverable site map per [llmstxt.org](https://llmstxt.org/). Pointers to install commands, demo, /docs, repo, legal pages. Served at `https://passporttowealth.app/llms.txt`. |
...
| `publish-landing.sh` | **Use this to publish the landing page.** Regenerates `docs/index.html`, substitutes `{{BUILD_STAMP}}` in a temp build dir, then publishes to here.now. Don't invoke the here-now skill against `installer/` directly — the timestamp would render literally. |
...
`installer/docs/index.html` is the live "What this installs" page at `https://passporttowealth.app/docs/`. Multi-section, here.now-style sidebar nav. Six sections: Quick start, What you get, Privacy at a glance, What this installs (auto-extracted), Sharing your dashboard, Reference (env vars + common questions).

It's regenerated by `python3 installer/build_docs.py`. The script:

- Parses `installer/install.sh` and `installer/install.ps1` for every package, URL, env var, and workspace path.
- Cross-references them against curated description maps (`TOOLS`, `PYTHON_PACKAGES`, `URLS`, `FILE_OPERATIONS_DESCRIPTIONS`, `ENV_VAR_DESCRIPTIONS`) inside the script itself.
- **Fails loudly if anything in the install scripts isn't in the curated maps.** Adding a new `brew install`, `winget install`, `npx skills add`, fetched URL, or env var requires an entry in the relevant map.

To update: edit the install scripts as normal; run `python3 installer/build_docs.py`; commit the regenerated `docs/index.html`. CI runs `--check` mode and fails the PR if you forgot. `publish-landing.sh` also regenerates before publishing, so the live page can never be stale.

## Legal pages

`installer/legal/` contains three pages: `privacy.html`, `terms.html`, `dpa.html`. All are **draft pending legal review** — engineering drafts based on the actual data flows in the code, written so testers and clients have something honest to read until counsel signs off.

- **Don't edit them lightly.** A factual correction (e.g., "we now contact a new third-party service" → update the sub-processors table) is fine. Wholesale rewrites or new clauses need the user's go-ahead.
- **Keep the draft banner** at the top of each page until counsel formally signs off. Removing the banner without authorization changes the legal posture.
- The pages share inline CSS (matched to `/docs`) for portability. `installer/legal/_shell.html` is a placeholder/comment; not served.
- Linked from the landing page footer (Legal column) and from `installer/llms.txt`.
