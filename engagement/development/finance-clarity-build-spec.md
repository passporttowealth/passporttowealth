...
| OP-8 | **Minimize user exposure to third-party service names, credentials, and technical plumbing.** The agent owns every third-party interaction it can. **v1 exception:** during install, the user is walked through one publishing-host signup (the agent opens the browser tab for them and they paste the resulting key once). After install, the user never sees the service name, the key, slugs, venv paths, or package names again. | User-facing strings outside the install dialog reviewed against a banned-words list: `here.now`, `API key`, `credential`, `slug`, `Homebrew`, `Python`, `pip`, `venv`, `npx`, `OFX`, `webhook`. Permitted: "your site", "your passcode", "your folder", "your dashboard". The full "agent owns the publishing host signup with no user involvement" path is a v2 backlog item (per-client signed install links, scoped credentials, handshake endpoint). |
...
| OP-11 | **Refuse to run in environments where the skill cannot uphold its own guarantees.** Pre-flight checks at install and at every START-HERE: managed-device (MDM) detection, free-disk minimum (5 GB), Mac architecture, network reachability for FX + publishing host, Claude Code authentication state, Anthropic subscription status (Pro/Max) **or** valid `ANTHROPIC_API_KEY`. Any failed check aborts with a plain-English explanation and the support bundle path. | Single `preflight()` function callable from installer + START-HERE. Returns structured failures mapped to FCB-00xx codes. |
...
| OP-13 | **Anthropic data-terms consent is required before any install action runs.** The installer presents a plain-English explanation of what Claude is, what data is sent to Anthropic during normal use, and links to Anthropic's privacy hub (`https://privacy.anthropic.com/`), privacy policy (`https://www.anthropic.com/legal/privacy`), consumer/commercial terms, and trust center (`https://trust.anthropic.com/`). Auto-opens the privacy hub in the browser. The user must type explicit acceptance (`I accept` / `accept` / `agree` / `yes`) — anything else cancels the install with no side effects. Acceptance is logged with timestamp. | First gate in `Welcome.command`, before pre-flight (OP-11). Refusal exits cleanly with zero file-system writes outside `install.log`. The same consent message and link list are surfaced again as a one-time read-back when the skill first launches. |
...
- **A clearly highlighted "Mac will warn you — that's normal" callout** with three numbered steps and a screenshot:
  ...
  2. Open Finder, go to your Downloads folder. **Right-click** `Welcome.command` and choose **Open**.
  3. Mac asks again, this time with an **Open** button. Click it.
...
When the client double-clicks `Welcome.command`:

1. **Open Terminal** automatically. Detect Xcode Command Line Tools presence and adjust the time estimate before showing it:
   - Xcode CLT already installed → title bar: "Setting up your finance workspace — about 10 minutes."
   - Xcode CLT missing → "Setting up your finance workspace — 30 to 60 minutes (Mac needs to download some developer tools first)."
2. **Run pre-flight (OP-11)** before touching anything:
...
3. **Show a plain-language progress log only.** No package names, no shell commands, no version strings. The user sees, e.g., "Installing the tools your dashboard needs… ✓".
4. **Provision the runtime silently** (technical steps hidden, but each one logged to `install.log` for support):
   - Install Xcode Command Line Tools if absent (Mac shows its native consent dialog — unavoidable but Apple-native).
   - Install the package manager if absent. Asks for the Mac password once. On-screen: "Mac is asking permission to install your tools — this is normal."
   - Install the language runtime, JSON helper, and required libraries (into a workspace-local virtualenv so nothing global is touched).
   - Install the AI assistant CLI.
   - Install both companion skills (the publishing-host skill and `finance-clarity-build`).
5. **Authenticate the AI assistant** (§4.3) — first third-party touchpoint.
6. **Walk the user through the publishing-host signup** (§4.4) — second third-party touchpoint.
7. **Finalize:**
   ...
   - Drop `START-HERE` on the Desktop, pointing at the actual workspace location. `Welcome.command` self-deletes on success.
8. **Verify** end-to-end with the diagnostic (§4.5) and report green.
9. **Done message:** "✓ Your workspace is ready. Double-click START-HERE on your Desktop whenever you want to use it."
...
The pasted key is written to the workspace's `.env` (alongside the publishing-host passcode later) **and** exported into START-HERE's launch environment as `ANTHROPIC_API_KEY`. Verified with a test API call (`POST /v1/messages` with `max_tokens: 1`) before continuing. Failure → plain message "that key didn't work — paste it again, or check your Anthropic billing."
...
### 4.4 Publishing-host signup (the second third-party touchpoint)

The installer pauses, opens the publishing-host signup page in the user's default browser, and shows in Terminal:

```
I need a free account on the service that will host your private dashboard.
I've opened it in your browser. Please:

  1. Sign up — just an email and password. (You'll only do this once.)
  2. CHECK YOUR EMAIL for a verification link if asked. Click it. Come back.
  3. On the page that opens, look for a section called "API Keys"
     (might be under Settings or Account). Click "Create New Key".
  4. Copy the long string of letters and numbers. It looks like:
        hn_live_aBcD1234efGh5678…
  5. Paste it here, then press Enter.

(If you'd rather have your advisor do this, just call them now and
share your screen. They can paste it for you. Take your time.)
```

#### 4.4.1 Validation
Pasted value is trimmed of whitespace. Verified with a test API call (a no-op metadata fetch). Three failed pastes triggers: "I'm not getting a working key. Want me to wait while you call your advisor?" — installer pauses indefinitely until the user presses Enter to retry or types `quit`.

#### 4.4.2 Common failure modes the agent catches
- **User pasted their email instead of the key** → "That looks like an email address. The key is a long random string starting with `hn_`."
- **User pasted the dashboard URL** → "That looks like a web address. Look for 'API Keys' on the page, not the URL bar."
- **Key is rejected as invalid** → "The service says that key isn't recognized. Did you confirm your email yet?"
- **Key is valid but rate-limit / billing error** → "Your account exists but the service says it's not activated yet. Check your email for a confirmation."

Pasted key is written to `~/.herenow/credentials` with `chmod 600`. Never echoed to logs. After this step the user never hears about the publishing host again — the agent reads the key from the locked file every time it publishes.

### 4.5 `START-HERE` and the diagnostic

`START-HERE` is a single double-clickable file on the Desktop. Behind the scenes it:

1. Acquires a single-instance lock (`~/Documents/my-finances/.lock`). If another instance is running, focus its window and exit.
2. Activates the runtime (Terminal scrollback for activation suppressed).
3. Exports `ANTHROPIC_API_KEY` from `.env` if API-key auth was chosen at install.
4. Opens a Finder window pointing at `inbox/` so the user has a visible target to drop files into.
5. Launches the AI assistant. The user sees the assistant's greeting (§6.1).

The diagnostic (per OP-11) runs automatically at the start of every START-HERE launch. If anything is red, the assistant says: "Something's not right on your laptop. I've put together what my advisor needs to help — I'll walk you through sending it." The agent then runs the support bundle path (§17.6).

### 4.3 Publishing-host signup (the one third-party moment)
...
I've opened it in your browser. Please:
  ...
  1. Sign up (just an email and password — you'll only do this once).
  2. Find your "API key" on the dashboard page that appears.
  3. Copy it and paste it here, then press Enter.

(If you'd rather have your advisor do this, just call them now and
share your screen. They can paste it for you.)
```

The pasted key is written to `~/.herenow/credentials` with `chmod 600`. Never echoed to logs. Verified with a test API call before the installer continues. After this step the user never hears about the publishing host again — the agent reads the key from the locked file every time it publishes.

If the user prefers, the advisor (live, screen-sharing during the demo per `demo-script.md` Phase 1) can paste the key on their behalf.

### 4.4 `START-HERE` and the diagnostic

`START-HERE` is a single double-clickable file on the Desktop. Behind the scenes it opens Terminal in the workspace, activates the runtime, and launches the AI assistant. The user sees the assistant's greeting (§6.1) — Terminal scrollback for the runtime activation is suppressed.

The diagnostic runs automatically on every start. If anything is red, the assistant says: "Something's not right on your laptop. I've put together what my advisor needs to help — I'll walk you through sending it." The agent then runs the support bundle path (§17.6).

### 4.6 Cross-platform parity (Mac is reference; Windows is a parallel target)

The repo ships installer scaffolding for **both Mac and Windows** from v0:

- **Mac** (reference v1): `installer/Welcome.command` (Bash). Self-relaunches in Terminal when double-clicked from Finder. macOS 13+ required.
- **Windows** (parallel target): `installer/Welcome.bat` (CMD wrapper) + `installer/Welcome.ps1` (PowerShell — the real logic). Windows 10 build 1903+ or Windows 11. Same behavior contract as Mac, mirrored step-for-step.

The landing page (`installer/index.html`) detects the visitor's OS and offers the right downloads + the right "your OS will warn you" instructions (Gatekeeper for Mac; SmartScreen for Windows). Both warnings are the single most common abandonment point, so both have explicit screenshot+steps callouts.

**Sync rule:** any user-facing string change in `Welcome.command` lands in `Welcome.ps1` in the same PR. CI lint will eventually enforce this; for now it's a code-review check. Linux is out of scope until a real customer asks for it.
...
- First prompt after `START-HERE.command` (greeting flow).
...
START-HERE has already opened a Finder window pointing at `inbox/` (§4.5 step 4). The greeting refers to it explicitly:
